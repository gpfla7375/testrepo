...
- 함수의 사용 : 함수 호출(function call)
    함수가 만들어진 형태(signature)에 맞춰서 사용해야 함
     ...
     => 매개변수의 갯수일치되게 전달인자(argument) 사용해야 함
     => 반환값 여부도 확인
        반환값 있는 함수는 함수(중첩함수())
'''
# 아무런 기능이 없는 (처리할 코드가 준비중인) 빈 함수를 만들때는 pass 사용
def func():
    pass

# 함수 이름 아래에 함수 설명(description)을 적어둘 수 있음. 따옴표 사용햠
def hello():
    '이 함수는 함수 작성 연습용입니다.'
    print('Welcome')
...
#
#     n1, n2 = func2(3.3, 24.5)
#     print(n1, type(n1), n2, type(n2))

# ------------------------------------------------------------------------
# 변수 생성과 사용 영역 (지역, scope)
...
def func_global():
    ...
    print('전역 변수 gnum : ', gnum)

# 함수의 매개변수(parameter)는 전달받은 값을 변경할 수 없다
# 전달받은 값이 군집자료형일 때는 아이템(요소)는 변경할 수 있음
def list_func(plist):
