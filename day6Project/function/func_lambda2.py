...
# map 내장함수 :
# 맵객체변수 : map(실행할 함수명, 시퀀스 객체)
# 시퀀스(Sequence) 객체 : 값을 순차적으로 저장하는 객체, 리스트나 튜플이 해당됨
# 시퀀스 객체의 각 요소값을 하나씩 꺼내서 함수로 보내고, 처리결과를 리턴받아서 맵객체에 저장하는 함수
def func(x):
    return x * x
...
# 시퀀스 객체의 각 요소의 값에 대해 함수 처리 결과가 참(True)인 요소만 골라서 저장함
def func1(x):
    return x > 2
def test_filter():
    nlist = [1, -2, 4, 7]
...
def test_sorted():
    students = [('김영희','A',95), ('이철', 'B', 84), ('이룸', 'C', 70)]
    print(students)    
