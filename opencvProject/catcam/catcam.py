...
# 오버레이 처리 함수
# 영상 프레임 이미지, 고양이귀 이미지, 고양이 귀 표시할 위치정보 전달받아서 오버레이에서 출력 처리함
def overlay(frame, cat, pos):
    ...
    img1[:, :, 0] = (img1[:, :, 0] * alpha + img2[:, :, 0] * (1. - alpha)).astype(np.int8)
    img1[:, :, 1] = (img1[:, :, 1] * alpha + img2[:, :, 1] * (1. - alpha)).astype(np.int8)
    img1[:, :, 2] = (img1[:, :, 2] * alpha + img2[:, :, 2] * (1. - alpha)).astype(np.int8)
