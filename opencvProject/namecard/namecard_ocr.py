...
# 함수 만들기
def reorderPts(pts):
    idx = np.lexsort((pts[:, 1], pts[:, 0])) # 컬럼0 --> 컬럼 1 순위로 정렬한 인덱스가 반환
    pts = pts[idx]
...
filename = r'../images/namecard1.jpg'

# 이 파일을 실행파일로 만들어서, 외부에서 실행시 전달인자로 이미지 파일을 전달하는 경우에 대한 처리
if len(sys.argv) > 1:
    filename = sys.argv[1] # 프롬포트 > 실행파일명 처리할 이미지파일명.확장자
...
# 출력 영상 설정
dw, dh = 720, 400
srcQued = np.array([[0, 0], [0, 0], [0, 0], [0, 0]], np.float32)
dstQued = np.array([[0, 0], [0, dh], [dw, dh], [dw, 0]], np.float32)
...
for pts in contours:
    ...
    srcQued = reorderPts(approx.reshape(4, 2).astype(np.float32))

    # 명함 이미지 4각형 이미지 만들기
    per = cv2.getPerspectiveTransform(srcQued, dstQued)
