...
# dsize 를 이미지 배율의 기준점 지정 (0, 0)으로 지정함, fx, fy : scaling factor (크기 배율) : 1.0 원래 크기, 축소 < 1.0 < 확대
cv2.imshow('src1', src)
...
# 히스토그램은 이미지의 밝기 분포를 그래프로 표현할 수 있음
# 그레이스케일 채널수 1개 [0], BG 색상채널이 2개이면 [0, 1], BGR 색상채널이 3개 [0, 1, 2]
hist = cv2.calcHist([src], [0], None, [256], [0, 256])
# 이미지 소스는 반드시 리스트 형태로 입력할 것 : [src]
# cv2.calcHist([images], chennels, mask, histSize, ranges[, hist, [, accumulate]])
# images : 분석대상 이미지 (uint8 or float32 type), Array 형태
# channels : 분석할 색상 채널 (x 축 대상)
