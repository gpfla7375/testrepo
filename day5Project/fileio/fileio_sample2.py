...
def test_binary_fio2():
    ...
    f.close()
    print(read_data)
    print(type(read_data)) # wb 는 객체타입 그대로 기록함
...
def test_os():
    ...
    print('current_path : ', os.path.basename(current_path)) # 파일명, 확장자 추출
    print('current_path : ', os.path.dirname(current_path)) # 경로명만 추출
    print('current_path : ', os.path.split(current_path)) # ('경로명', '파일명.확장자') 분리

    # path.splitdrive(), splittext() : 경로에서 드라이브명만, 확장자명만 추출
